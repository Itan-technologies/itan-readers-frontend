...
export default function ContributorsForm() {
  const [contributors, setContributors] = useState([{ role: 'Author', firstName: '', lastName: '' }]);

  const handleAdd = () => {
    if (contributors.length < 9) {
      setContributors([...contributors, { role: 'Author', firstName: '', lastName: '' }]);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 bg-white shadow rounded space-y-4">
      ...
      <p className="text-sm text-gray-600">Add up to 9 contributors. They’ll display on Amazon using the order you enter below.</p>
